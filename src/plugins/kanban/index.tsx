...
const SortableCard = ({ card, onDelete, onEdit }: { card: KanbanCard; onDelete: () => void; onEdit: (val: string) => void }) => {
  ...
  const priorityColors = { low: 'bg-green-500/20 text-green-500', med: 'bg-yellow-500/20 text-yellow-500', high: 'bg-red-500/20 text-red-500' };

  return (
    <div ref={setNodeRef} style={style} className="group relative flex flex-col gap-2 rounded-lg border border-white/10 bg-white/5 p-3 shadow-sm hover:border-blue-500/50">
      <div className="flex items-start justify-between">
        <div {...attributes} {...listeners} className="cursor-grab text-white/20 hover:text-white/40"><GripVertical size={14} /></div>
        <div className={`rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider ${priorityColors[card.priority]}`}>{card.priority}</div>
      </div>
      <div className="min-h-[1em] text-sm text-white/80 outline-none" contentEditable suppressContentEditableWarning onBlur={(e) => onEdit(e.currentTarget.innerText)}>{card.content}</div>
      <button onClick={onDelete} className="absolute bottom-2 right-2 text-white/0 hover:text-red-400 group-hover:text-white/20"><Trash2 size={12} /></button>
    </div>
  );
};
...
const SortableColumn = ({ column, cards, onAddCard, onUpdateColumn, onDeleteColumn, onUpdateCard, onDeleteCard }: any) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex h-fit min-h-[150px] w-72 min-w-[280px] flex-col gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-4 backdrop-blur-md">
      <div className="flex items-center justify-between">
        <div {...attributes} {...listeners} className="cursor-grab font-bold text-white/60 hover:text-white">
          <h3 className="outline-none" contentEditable suppressContentEditableWarning onBlur={(e) => onUpdateColumn({ ...column, title: e.currentTarget.innerText })}>{column.title}</h3>
        </div>
        <button onClick={onDeleteColumn} className="text-white/20 hover:text-red-400"><Trash2 size={14} /></button>
      </div>
      <SortableContext items={column.cardIds} strategy={verticalListSortingStrategy}>
        <div className="flex flex-col gap-3">
          {column.cardIds.map((cid: string) => <SortableCard key={cid} card={cards[cid]} onDelete={() => onDeleteCard(cid)} onEdit={(val) => onUpdateCard(cid, { content: val })} />)}
        </div>
      </SortableContext>
      <button onClick={onAddCard} className="mt-2 flex items-center justify-center gap-2 rounded-lg bg-white/5 py-2 text-xs font-bold text-white/40 hover:bg-white/10 hover:text-white"><Plus size={14} /> Add Card</button>
    </div>
  );
};
...
const KanbanBoardComponent = ({ node, updateAttributes, deleteNode }: any) => {
  ...
  const sync = (newData: KanbanBoardData) => {
    ...
    setData(validated);
    updateAttributes({ boardData: validated });
  };
  ...
  return (
    <NodeViewWrapper className="kanban-block-v3 my-8 rounded-2xl border border-white/10 bg-[#0f172a] p-6">
      <NexusErrorBoundary onRecover={() => deleteNode()}>
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-500/20 text-blue-500"><Layout size={20} /></div>
          <div><h2 className="text-lg font-bold text-white">Project Kanban</h2><p className="text-xs text-white/40 italic">Collaboration Active • Zod Validated</p></div></div>
        </div>
...
const KanbanExtension = Node.create({
  name: 'kanbanBoard',
  group: 'block',
  atom: true,
  addAttributes() { return { boardData: { default: { columns: [{ id: 'col-1', title: 'To Do', cardIds: [] }, { id: 'col-2', title: 'Done', cardIds: [] }], cards: {} } } }; },
  parseHTML() { return [{ tag: 'div[data-type="kanban-board"]' }]; },
  renderHTML({ HTMLAttributes }) { return ['div', mergeAttributes(HTMLAttributes, { 'data-type': 'kanban-board' })]; },
  addNodeView() { return ReactNodeViewRenderer(KanbanBoardComponent); },
});

const KanbanPlugin: NexusPlugin = {
  id: 'kanban-plugin',
  name: 'Kanban Boards',
  version: '3.20.0',
  extensions: [KanbanExtension],
};
